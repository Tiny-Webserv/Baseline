...
std::string Response::fetchFilePath() {
    ...
    if (target.size()) {
        ...
        for (size_t i = 0; i < index.size(); i++) {
			...
			{
				std::cout << "======================" << std::endl;
				std::cout << "bf2 return = " << target << std::endl;
				std::cout << "======================" << std::endl;
                return target;
			}
        }
	...
	std::cout << "======================" << std::endl;
	std::cout << "bf return = " << target << std::endl;
	std::cout << "======================" << std::endl;
...
int Response::generatePhpResponse(struct kevent *curEvnts,
    ...
    close(curEvnts->ident); // 우선은 강제 종료 느낌으로 처리 이거 이벤트 왜
                            // 삭제 안되는지 알아보면 좋을 듯
...
std::map<std::string, std::string> Response::PhpEnvSet() {
    ...
    std::string query_string = "QUERY_STRING="; // get 일 때 필요
